...
class VentanaCuadradosMedios(VentanaBase):
    ...
    def crear_widgets(self):
        ...
        frame_entrada.pack(pady=10, fill=tk.X, padx=10)

        ttk.Label(frame_entrada, text="Semilla (X0):").grid(row=0, column=0, padx=5, pady=5)
        self.semilla_entry = ttk.Entry(frame_entrada)
        ...
        self.iter_entry = ttk.Entry(frame_entrada)
        self.iter_entry.grid(row=0, column=3, padx=5, pady=5)

        ttk.Button(frame_entrada, text="Generar", command=self.generar_tabla).grid(row=0, column=4, padx=5, pady=5)

        self.frame_tabla = ttk.Frame(self)
        self.frame_tabla.pack(fill=tk.BOTH, expand=True, padx=10, pady=5)
    ...
    def generar_tabla(self):
        try:
            ...
            data = {"Iteración": [fila[0] for fila in tabla], "ri": [fila[4] for fila in tabla]}
    ...
    def exportar_excel(self):
        try:
            ...
            n = int(self.iter_entry.get())
            tabla = cuadrados_medios(semilla, n)
            data = [{"Iteración": fila[0], "Xi": fila[1], "Resultado": fila[2], "Xi+1": fila[3], "ri": fila[4]} for fila in tabla]
            exportar_a_excel(data, "cuadrados_medios.xlsx")
        except Exception as e:
            messagebox.showerror("Error", f"No se pudo exportar: {e}")

# --- CLASE PARA PRODUCTOS MEDIOS ---
class VentanaProductosMedios(VentanaBase):
    ...
    def crear_widgets(self):
        ...
        self.iter_entry = ttk.Entry(frame_entrada)
        self.iter_entry.grid(row=0, column=5, padx=5, pady=5)
    ...
    def exportar_excel(self):
        try:
            ...
            n = int(self.iter_entry.get())
            tabla = productos_medios(x0, x1, n)
            data = [{"Iteración": fila[0], "Xi": fila[1], "Xi+1": fila[2], "Resultado": fila[3], "Xi+2": fila[4], "ri": fila[5]} for fila in tabla]
            exportar_a_excel(data, "productos_medios.xlsx")
        except Exception as e:
            messagebox.showerror("Error", f"No se pudo exportar: {e}")

# --- CLASE PARA MULTIPLICADOR CONSTANTE ---
